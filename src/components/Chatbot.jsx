...
const USER_AVATAR = "https://ui-avatars.com/api/?name=You&background=34d399&color=fff&rounded=true";
  ...
  return (
    <div className="w-full max-w-2xl h-[36rem] bg-gradient-to-br from-green-50 to-white rounded-3xl shadow-2xl flex flex-col border-2 border-green-200">
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-green-100 bg-white rounded-t-3xl">
        <img src={BOT_AVATAR} alt="Bot" className="w-10 h-10 rounded-full border-2 border-green-200" />
        <div>
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-transparent">
        {messages.map((msg, idx) => (
          <div
            key={idx}
            className={`flex items-end gap-2 ${msg.from === "user" ? "justify-end" : "justify-start"} animate-fade-in`}
          >
            {msg.from === "bot" && (
              <img src={BOT_AVATAR} alt="Bot" className="w-8 h-8 rounded-full border border-green-200" />
            )}
            <span
              className={
                msg.from === "user"
            ...
            {msg.from === "user" && (
              <img src={USER_AVATAR} alt="You" className="w-8 h-8 rounded-full border border-green-200" />
            )}
          </div>
        ))}
